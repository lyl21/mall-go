...
func (i *initMenuAuthority) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	for _, menu := range allMenus {
		...
		// 兼容业务模块：为测试角色补充门店、商城、验光菜单权限，避免出现“当前此角色无当前路由”
		if menu.ParentId > 0 && (parentName == "systemTools" || parentName == "example" || parentName == "store" || parentName == "mall" || parentName == "optometry") {
			menu9528 = append(menu9528, menu)
		}
	}
